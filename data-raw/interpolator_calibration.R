...
topo_path <- "data-raw/penbal_topo_500.gpkg"
...
tar_load(
  calibrations#,
  # branches = seq(1, 370, length.out = 12) |> round(0) |> as.integer()
)
calibration_params_table <- calibrations |>
  purrr::list_rbind() |>
  dplyr::arrange(
    year, month, day, i_step
  )
...
calibration_params_table |>
  dplyr::group_by(i_step) |>
  dplyr::summarise(
    dplyr::across(
      dplyr::ends_with("mae"),
      list(mean = mean, median = median, min = min, max = max)
    ),
    topo_bbox = dplyr::first(topo_bbox)
  ) |>
  ...
  # dplyr::filter(n_stations < 51) |>
  ggplot() +
  geom_sf(aes(fill = tmax_mae_max, color = tmax_mae_max), alpha = 1) +
  # facet_wrap(vars(year, month)) +
  theme_minimal()
