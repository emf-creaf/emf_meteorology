...
calculate_daily_averages <- function(gpkg_file, admin_level) {
    ...
    \(name, cpro, geom) {
      sf::st_read(gpkg_file, wkt_filter = sf::st_as_text(geom), quiet = TRUE) |>
        ...
        dplyr::summarise(
          ...
          MinRelativeHumidity = mean(MinRelativeHumidity, na.rm = TRUE),
          MaxRelativeHumidity = mean(MaxRelativeHumidity, na.rm = TRUE),
          Radiation = mean(Radiation, na.rm = TRUE),
          WindSpeed = mean(WindSpeed, na.rm = TRUE),
          WindDirection = mean(WindDirection, na.rm = TRUE),
          PET = mean(PET, na.rm = TRUE),
          ThermalAmplitude = mean(ThermalAmplitude, na.rm = TRUE),
          .by = "dates"
        ) |>
        dplyr::mutate(
          name = name,
